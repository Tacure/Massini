...
namespace Massini.Flamet.Sugar.Classes
{
    public unsafe class SmartShaderLink : IResource, IDisposable
    {
        ...
        public void Bind(CommonEncoder i_encoder, SmartShaderLinkParams i_params)
        {
            ...
            {
                NativeMemory.Clear(fingerprintsPtr, (nuint)(sizeof(SetFingerprint) * ApiConstants.MAX_SETS_PER_PIPELINE));
            }

            // Take fingerprints.
            TakeFingerprints(i_params, m_fingerprints, m_resources);

            for (int setNumber = 0; setNumber < ApiConstants.MAX_SETS_PER_PIPELINE; setNumber++)
            {
            ...
            foreach (var stateTuple in m_sets)
            {
                ...
                if (state.TakenCommandLists.Count > 0)
                {
                    ...
                    foreach (Rid commandListId in m_commandListsToRemove)
                    {
        ...
        private readonly SetFingerprint[] m_fingerprints = new SetFingerprint[ApiConstants.MAX_SETS_PER_PIPELINE];
