...
using Massini.Flamet.Structs.Level1.Internal;
...
namespace Massini.Flamet.Classes
{
    ...
    public unsafe partial class CommandList : IResource, IDisposable
    {
        ...
        /// <summary>
        /// The queue family this command list belongs to.
        /// </summary>
        public QueueFamily QueueFamily => m_queueFamily;

        // TODO: Add a clear explanation of what this does.
        ...
        /// </summary>
        /// <remarks>
        /// Command lists can only be used by queues that belong to the queue family they were created with.
        /// </remarks>
        public CommandList(QueueFamily i_queueFamily, in CommandListCreateParams i_createParams)
        {
            ...
            IntSharedFuncs.SetObjectLabel(i_queueFamily.Device, commandBuffer, VkObjectType.VK_OBJECT_TYPE_COMMAND_BUFFER, $"{nameof(CommandList)} - {i_createParams.p_label}");
            IntSharedFuncs.SetObjectLabel(i_queueFamily.Device, semaphore, VkObjectType.VK_OBJECT_TYPE_SEMAPHORE, $"{nameof(CommandList)} - Semaphore - {i_createParams.p_label}");
        ...
        /// <summary>
        /// Waits for the command list to finish executing. (GPU -> CPU synchronization).
        /// </summary>
        ...
        /// <summary>
        /// Writes all recorded commands to the internal command buffer.
        /// </summary>
        public void Flush()
        {
            m_isFlushed = true;
        ...
        /// <summary>
        /// Submits the command list to the queue.
        /// </summary>
    ...
    public unsafe partial class CommandList 
    {
        ...
        private static void FillTextureBarrierMipmapsBitmask(in TextureSubresourceLayers i_region, ulong[] i_masks)
        {
        ...
        private static void FillTextureBarrierMipmapsBitmaskToAll(ulong[] i_masks)
        {
        ...
        private void HandleCmdRenderPass(CmdRenderPass i_cmdRenderPass)
        {
            ...
            for (int i = 0; i < renderPassBeginParams.p_colorAttachments.Length; i++)
            {
                ...
                clearColorValue.float32[0] = renderPassColorAttachment.p_clearColor.p_x;
                clearColorValue.float32[1] = renderPassColorAttachment.p_clearColor.p_y;
                clearColorValue.float32[2] = renderPassColorAttachment.p_clearColor.p_z;
                clearColorValue.float32[3] = renderPassColorAttachment.p_clearColor.p_w;
                ...
                VkRenderingAttachmentInfo colorAttachment = new()
                {
                    ...
                    loadOp = IntSharedCvs.LoadOpToVkAttachmentLoadOp(renderPassColorAttachment.p_loadOp),
                    storeOp = IntSharedCvs.StoreOpToVkAttachmentStoreOp(renderPassColorAttachment.p_storeOp),
                    clearValue = new()
                    {
            ...
            if (renderPassBeginParams.p_depthStencilAttachment.HasValue)
            {
                ...
                depthAttachment = new()
                {
                    ...
                    loadOp = IntSharedCvs.LoadOpToVkAttachmentLoadOp(renderPassDepthStencilAttachment.p_depthLoadOp),
                    storeOp = IntSharedCvs.StoreOpToVkAttachmentStoreOp(renderPassDepthStencilAttachment.p_depthStoreOp),
                    clearValue = new()
                    {
                        depthStencil = new()
                        {
        ...
        private void HandleCmdCopyBufferToTexture(CmdCopyBufferToTexture i_cmdCopyBufferToTexture)
        {
            ...
            FillTextureBarrierMipmapsBitmaskToAll(dstMasks);
            ...
            for (int i = 0; i < i_cmdCopyBufferToTexture.p_regions.Length; i++)
            {
                vkBufferImageCopies[i] = new()
                {
                    ...
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdCopyBufferToTexture.p_regions[i].p_textureSubresource.p_aspectMask),
        ...
        private void HandleCmdCopyTextureToTexture(CmdCopyTextureToTexture i_cmdCopyTextureToTexture)
        {
            ...
            for (int i = 0; i < i_cmdCopyTextureToTexture.p_regions.Length; i++)
            {
                ...
                FillTextureBarrierMipmapsBitmask(in region.p_srcSubresource, srcMasks);
                FillTextureBarrierMipmapsBitmask(in region.p_dstSubresource, dstMasks);
                ...
                vkImageCopies[i] = new()
                {
                    ...
                    srcSubresource = new()
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdCopyTextureToTexture.p_regions[i].p_srcSubresource.p_aspectMask),
                    ...
                    dstSubresource = new()
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdCopyTextureToTexture.p_regions[i].p_dstSubresource.p_aspectMask),
        ...
        private void HandleCmdCopyTextureToBuffer(CmdCopyTextureToBuffer i_cmdCopyTextureToBuffer)
        {
            ...
            ulong[] srcMasks = new ulong[srcTexture.ArrayLayersCount];
            FillTextureBarrierMipmapsBitmaskToAll(srcMasks);
            ...
            for (int i = 0; i < i_cmdCopyTextureToBuffer.p_regions.Length; i++)
            {
                vkBufferImageCopies[i] = new()
                {
                    ...
                    imageSubresource = new()
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdCopyTextureToBuffer.p_regions[i].p_textureSubresource.p_aspectMask),
        ...
        private void HandleCmdBlitTexture(CmdBlitTexture i_cmdBlitTexture)
        {
            ...
            for (int i = 0; i < i_cmdBlitTexture.p_regions.Length; i++) 
            {
                ...
                vkRegions[i] = new()
                {
                    srcSubresource = new()
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdBlitTexture.p_regions[i].p_srcSubresource.p_aspectMask),
                    ...
                    dstSubresource = new() 
                    {
                        aspectMask = (uint)IntSharedCvs.TextureAspectFlagsToVkImageAspectFlagBits(i_cmdBlitTexture.p_regions[i].p_dstSubresource.p_aspectMask),
            ...
            fixed (VkImageBlit* vkRegionsPtr = vkRegions)
            {
                Vk.vkCmdBlitImage(
                    ...
                    vkRegionsPtr,
                    IntSharedCvs.FilterModeToVkFilter(i_cmdBlitTexture.p_filterMode));
            }
        }
        ...
        private void HandleCmdBindIndexBuffer(CmdBindIndexBuffer i_cmdBindIndexBuffer)
        {
            ...
            Vk.vkCmdBindIndexBuffer(
                ...
                IntSharedCvs.IndexFormatToVkIndexType(i_cmdBindIndexBuffer.p_indexFormat));
        }
        ...
        private void HandleCmdPushSet(CmdPushSet i_cmdPushPipelineSet, bool i_isComputePass)
        {
            ...
            for (int i = 0; i < i_cmdPushPipelineSet.p_bindingDescription.Length; i++)
            {
                ...
                if (binding.p_bufferBinding.HasValue)
                {
                    ...
                    vkDescriptorTypesListBuffers.Add(IntSharedCvs.EntryTypeToVkDescriptorType(binding.p_type));
                }
                else if (binding.p_textureBinding.HasValue)
                {
                    ...
                    vkDescriptorTypesListImages.Add(IntSharedCvs.EntryTypeToVkDescriptorType(binding.p_type));
                }
            }
        ...
        private void HandleCmdPushConstant(CmdPushConstant i_cmdPushConstant)
        {
            ...
            fixed (byte* ptr = i_cmdPushConstant.p_data)
            {
                Vk.vkCmdPushConstants(
                    m_ptr_commandBuffer,
                    pipelineLayoutPtr,
                    (uint)IntSharedCvs.ShaderStageFlagsToVkShaderStageFlagBits(i_cmdPushConstant.p_stageFlags),
                    0,
                ...
                for (int i = 0; i < setDeclaration.p_entries.Length; i++)
                {
                    ...
                    switch (entryDeclaration.p_type)
                    {
                        case EntryType.UniformBuffer:
                            ...
                            CmdBufferBarrier(
                                uniform,
                                IntSharedCvs.EntryModeToVkAccessFlagBits(entryDeclaration.p_mode),
                                IntSharedCvs.ShaderStageFlagsToVkPipelineStageFlagBits(entryDeclaration.p_stages));
                            break;
                        ...
                        case EntryType.Texture:
                            ...
                            ulong[] masks = new ulong[textureView.Texture.ArrayLayersCount];
                            FillTextureBarrierMipmapsBitmaskToAll(masks);
            ...
            for (int i = 0; i < i_bindingDescription.Length; i++) 
            {
                ...
                switch (i_bindingDescription[i].p_type)
                {
                    case EntryType.UniformBuffer:
                        ...
                        CmdBufferBarrier(
                            uniform,
                            IntSharedCvs.EntryModeToVkAccessFlagBits(usageInfo.p_mode),
                            IntSharedCvs.ShaderStageFlagsToVkPipelineStageFlagBits(usageInfo.p_stages));
                        break;
                    case EntryType.StorageBuffer:
        ...
        private void HandleCmdBindShaderLink(CmdBindShaderLink i_cmdBindShaderLink)
        {
            ...
            if (m_boundShaderLink != null && m_boundShaderLink != i_cmdBindShaderLink.p_shaderLink)
            {
                ...
                {
                    vkUnbindStages[i] = IntSharedCvs.ShaderStageFlagsToVkShaderStageFlagBits(m_boundShaderLink.Stages[i]);
                }

                fixed (VkShaderStageFlagBits* vkUnbindStagesPtr = vkUnbindStages)
                {
                    m_queueFamily.Device.GetFunctionTable().PfnVkCmdBindShadersExt(m_ptr_commandBuffer, (uint)vkUnbindStages.Length, vkUnbindStagesPtr, null);
                }
            }
            ...
            if (m_boundShaderLink != i_cmdBindShaderLink.p_shaderLink)
            {
                ...
                {
                    vkStages[i] = IntSharedCvs.ShaderStageFlagsToVkShaderStageFlagBits(stages[i]);
                }

                fixed (VkShaderStageFlagBits* vkStagesPtr = vkStages)
                fixed (VkShaderEXT_T** shaderPtrsPtr = shaderPtrs)
                {
                    m_queueFamily.Device.GetFunctionTable().PfnVkCmdBindShadersExt(m_ptr_commandBuffer, (uint)vkStages.Length, vkStagesPtr, shaderPtrsPtr);
                }
        ...
        {
            m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetPolygonModeExt(m_ptr_commandBuffer, IntSharedCvs.PolygonModeToVkPolygonMode(i_cmdSetPolygonMode.p_polygonMode));
        }

        private void HandleCmdSetRasterizationSamples(CmdSetRasterizationSamples i_cmdSetRasterizationSamples)
        {
            m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetRasterizationSamplesExt(m_ptr_commandBuffer, IntSharedCvs.SampleCountToVkSampleCountFlagBits(i_cmdSetRasterizationSamples.p_rasterizationSamples));
        }

        private void HandleCmdSetSampleMask(CmdSetSampleMask i_cmdSetSampleMask)
        {
            fixed (uint* maskPtr = i_cmdSetSampleMask.p_mask)
                m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetSampleMaskExt(
                m_ptr_commandBuffer, 
                IntSharedCvs.SampleCountToVkSampleCountFlagBits(i_cmdSetSampleMask.p_samples),
                maskPtr);
        }

        private void HandleCmdSetFrontFace(CmdSetFrontFace i_cmdSetFrontFace)
        {
            Vk.vkCmdSetFrontFace(m_ptr_commandBuffer, IntSharedCvs.FrontFaceToVkFrontFace(i_cmdSetFrontFace.p_frontFace));
        }

        private void HandleCmdSetPrimitiveTopology(CmdSetPrimitiveTopology i_cmdSetPrimitiveTopology)
        {
            Vk.vkCmdSetPrimitiveTopology(m_ptr_commandBuffer, IntSharedCvs.PrimitiveTopologyToVkPrimitiveTopology(i_cmdSetPrimitiveTopology.p_primitiveTopology));
        }
        ...
        {
            m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetDepthClampEnableExt(m_ptr_commandBuffer, i_cmdSetDepthClampEnable.p_depthClampEnable ? 1U : 0U);
        }

        private void HandleCmdSetAlphaToCoverageEnable(CmdSetAlphaToCoverageEnable i_cmdSetAlphaToCoverageEnable)
        {
            m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetAlphaToCoverageEnableExt(m_ptr_commandBuffer, i_cmdSetAlphaToCoverageEnable.p_alphaToCoverageEnable ? 1U : 0U);
        }
        ...
        private void HandleCmdSetColorBlendEnable(CmdSetColorBlendEnable i_cmdSetColorBlendEnable)
        {
            ...
            {            
                m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetColorBlendEnableExt(
                    m_ptr_commandBuffer, 
        ...
        private void HandleCmdSetColorWriteMask(CmdSetColorWriteMask i_cmdSetColorWriteMask)
        {
            ...
            {
                colorWriteMasks[i] = (uint)IntSharedCvs.ColorComponentFlagsToVkColorComponentFlagBits(i_cmdSetColorWriteMask.p_colorWriteMasks[i]);
            }

            fixed (uint* colorWriteMasksPtr = colorWriteMasks)
            {            
                m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetColorWriteMaskExt(
                    m_ptr_commandBuffer, 
        ...
        private void HandleCmdSetVertexInput(CmdSetVertexInput i_cmdSetVertexInput)
        {
            ...
            for (int i = 0; i < i_cmdSetVertexInput.p_vertexAttributesLayouts.Length; i++)
            {
                ...
                vertexInputBindingDescriptionArray[i] = new VkVertexInputBindingDescription2EXT
                {
                    ...
                    inputRate = IntSharedCvs.VertexStepModeToVkVertexInputRate(layout.p_stepMode),
                ...
                for (int j = 0; j < layout.p_attributes.Length; j++)
                {
                    ...
                    vertexInputAttributeDescriptionArray[attributeIndex++] = new VkVertexInputAttributeDescription2EXT
                    {
                        ...
                        binding = layout.p_binding,
                        format = IntSharedCvs.VertexFormatToVkFormat(attribute.p_format),
            ...
            {
                m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetVertexInputExt(
                    m_ptr_commandBuffer, 
        ...
        private void HandleCmdSetColorBlendEquation(CmdSetColorBlendEquation i_cmdSetColorBlendEquation)
        {
            ...
            {
                blendEquations[i].alphaBlendOp = IntSharedCvs.BlendOpToVkBlendOp(i_cmdSetColorBlendEquation.p_blendEquations[i].p_alphaBlendOp);
                blendEquations[i].colorBlendOp = IntSharedCvs.BlendOpToVkBlendOp(i_cmdSetColorBlendEquation.p_blendEquations[i].p_colorBlendOp);
                blendEquations[i].dstAlphaBlendFactor = IntSharedCvs.BlendFactorToVkBlendFactor(i_cmdSetColorBlendEquation.p_blendEquations[i].p_dstAlphaBlendFactor);
                blendEquations[i].dstColorBlendFactor = IntSharedCvs.BlendFactorToVkBlendFactor(i_cmdSetColorBlendEquation.p_blendEquations[i].p_dstColorBlendFactor);
                blendEquations[i].srcAlphaBlendFactor = IntSharedCvs.BlendFactorToVkBlendFactor(i_cmdSetColorBlendEquation.p_blendEquations[i].p_srcAlphaBlendFactor);
                blendEquations[i].srcColorBlendFactor = IntSharedCvs.BlendFactorToVkBlendFactor(i_cmdSetColorBlendEquation.p_blendEquations[i].p_srcColorBlendFactor);
            }

            fixed (VkColorBlendEquationEXT* blendEquationsPtr = blendEquations)
            {
                m_queueFamily.Device.GetFunctionTable().PfnVkCmdSetColorBlendEquationExt(
                    m_ptr_commandBuffer,
        ...
        {
            Vk.vkCmdSetDepthCompareOp(m_ptr_commandBuffer, IntSharedCvs.CompareOperationToVkCompareOp(i_cmdSetDepthCompareOp.p_depthCompareOp));
        }
