    ...
    public unsafe class ShaderLink : IResource, IDisposable
    {
        ...
        public ShaderLink(Device i_device, in ShaderLinkCreateParams i_createParams)
        {
            ...
            for (uint i = 0; i < i_createParams.p_stages.Length; i++)
            {
                ...
                HeapString nativeEntryPoint = HeapString.CreateUTF8(stage.p_entryPoint);
                entryPoints.Add(nativeEntryPoint);
                entryPointNames[i] = (sbyte*)nativeEntryPoint.RawPtr();
                ...
                if (i < i_createParams.p_stages.Length - 1)
                {
                    ...
                    if (!IsValidTransition(stage.p_stage, shadersNextStage[i]))
                    {
                ...
                fixed (VkDescriptorSetLayout_T** descriptorSetLayoutsPtr = i_createParams.p_layout.SetLayoutsPtrs)
                {
                    ...
                    for (int i = 0; i < i_createParams.p_stages.Length; i++)
                    {
                        ...
                        VkShaderCreateInfoEXT shaderCreateInfo = new()
                        {
                            ...
                            stage = IntSharedCvs.ShaderStageFlagsToVkShaderStageFlagBits(stage.p_stage),
                            nextStage = (uint)IntSharedCvs.ShaderStageFlagsToVkShaderStageFlagBits(nextStage),
                    ...
                    fixed (VkShaderCreateInfoEXT* shaderCreateInfosPtr = shaderCreateInfos)
                    {
                        ...
                        {
                            i_device.GetFunctionTable().PfnVkCreateShadersExt(i_device.VkDevicePtr, (uint)i_createParams.p_stages.Length, shaderCreateInfosPtr, null, shadersPtr);
                        }
                    }
                    ...
                    for (int i = 0; i < i_createParams.p_stages.Length; i++)
                    {
                        ...
                        IntSharedFuncs.SetObjectLabel(i_device, shader, VkObjectType.VK_OBJECT_TYPE_SHADER_EXT, $"{nameof(ShaderLink)} - {i_createParams.p_label} - {i_createParams.p_stages[i].p_stage}");
                    }
        ...
        public void Dispose()
        {
            ...
            {
                m_device.GetFunctionTable().PfnVkDestroyShaderExt(m_device.VkDevicePtr, m_shadersPtrs[i], null);
            }
        ...
        private static bool IsValidTransition(ShaderStageFlags i_current, ShaderStageFlags i_next)
        {
            return i_current switch
            {
                ShaderStageFlags.Vertex =>
                    i_next is ShaderStageFlags.Fragment or ShaderStageFlags.Geometry or ShaderStageFlags.TessControl,

                ShaderStageFlags.Geometry =>
                    i_next is ShaderStageFlags.Fragment,

                ShaderStageFlags.TessControl =>
                    i_next is ShaderStageFlags.TessEvaluation,

                ShaderStageFlags.TessEvaluation =>
                    i_next is ShaderStageFlags.Geometry or ShaderStageFlags.Fragment,

                ShaderStageFlags.Fragment =>
                    i_next is ShaderStageFlags.None,
